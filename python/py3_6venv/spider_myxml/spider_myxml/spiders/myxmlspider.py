...
class MyxmlspiderSpider(XMLFeedSpider):
    ...
    def parse_node(self, response, nodes):
        # log.msg('Hi, this is a <%s> node!: %s' % (self.itertag, ''.join(node.extract())))

        i = SpiderMyxmlItem()

        # 利用XPath表达式将对应信息提取出来，并存储到对应的Item中
        i['title'] = nodes.xpath("/rss/channel/item/title/text()").extract()
        i['link'] = nodes.xpath("/rss/channel/item/title/link()").extract()
        i['author'] = nodes.xpath("/rss/channel/item/author/text()").extract()
        print(len(i['title']))
